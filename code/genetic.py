...
class GeneticAlgorithm(algorithm.Algorithm):
    def __init__(self, explainer, variable, constant=None, n_grid_points=21, **kwargs):
        super().__init__(
            explainer=explainer,
            variable=variable,
            constant=constant,
    ...
    def fool(
        ...
        self._E_pop = np.tile(
            self.result_explanation["original"], (self._X_pop.shape[0], 1)
        )
        ...
        for i in pbar:
            ...
            pbar.set_description(
                "Iter: %s || Loss: %s" % (i, self.iter_losses["loss"][-1])
            )
        ...
        self.result_explanation["changed"] = self.explainer.pd(
            self.get_best_data(), self._idv, self.result_explanation["grid"]
        )

        _data_changed = pd.DataFrame(
            self.get_best_data(), columns=self.explainer.data.columns
        )
    ...
    def mutation(self, adjust=1):
        ...
        _theta = np.random.normal(
            loc=0, scale=self._X_std * adjust, size=(_temp_pop_count, self._n, self._p)
        )
        # preserve zeros
        _theta = np.where(self._X_pop == 0, 0, _theta)
        # column mask made with the probability
        _mask = np.random.binomial(
            n=1, p=self.params["mutation_prob"], size=(_temp_pop_count, 1, self._p)
        )
    ...
    def evaluation(self):
        self._E_pop = self.explainer.pd_pop(
            X_pop=self._X_pop, idv=self._idv, grid=self.result_explanation["grid"]
        )
        self._L_pop = loss.loss_pop(
            original=self.result_explanation["target"]
            if self._aim
            else self.result_explanation["original"],
            changed=self._E_pop,
            aim=self._aim,
            center=self._center,
        )
    ...
    def selection(self):
        ...
        self._X_pop = np.concatenate(
            (self._X_pop[_sorted_ids], self._X_pop[_top_f_ids])
        )
        self._L_pop = np.concatenate(
            (self._L_pop[_sorted_ids], self._L_pop[_top_f_ids])
        )
